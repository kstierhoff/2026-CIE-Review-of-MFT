...
tdr.summ.haul <- tdr.summ.haul %>% 
  ...
  mutate(distance.km = as.numeric(distance)/1000) %>%
  mutate(cpue.tot = catch.tot/distance.km,
         cpue.cps = catch.cps/distance.km,
         cpue.tot.v = catch.tot/as.numeric(V),
         cpue.cps.v = catch.cps/as.numeric(V)) %>% 
...
cpue.vol.hist.tot <- ggplot(filter(tdr.summ.haul, !cruise == "202103", 
                                    cpue.tot.v > 0), 
                        aes(log(cpue.tot.v+1), fill = gear.type)) + 
  geom_histogram(show.legend = FALSE) + facet_wrap(~cruise, ncol = 1) + 
  labs(title = "CPUE (All catch)",
       x = "log(CPUE + 1)",
       y = "Frequency") +
...
cpue.vol.hist.cps <- ggplot(filter(tdr.summ.haul, !cruise == "202103", 
                                    cpue.cps.v > 0), 
                        aes(log(cpue.cps.v + 1), fill = gear.type)) + 
  geom_histogram(show.legend = FALSE) + facet_wrap(~cruise, ncol = 1) + 
  labs(title = "CPUE (All catch)",
       x = "log(CPUE + 1)",
       y = "Frequency") +
