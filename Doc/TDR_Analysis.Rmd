...
# Add nav data to tdr.fishing
tdr.sub <- filter(tdr.fishing, cruise == cruise.name)
nav.paths <- nav %>% 
...
ggplot(tdr.nav.all, aes(long, lat, group = haul)) + geom_path() + coord_map() + facet_wrap(~cruise, nrow = 1)

tdr.nav.sf <- tdr.nav.haul %>% 
  st_as_sf(coords = c("long","lat"), crs = 4326) 
...
mapview::mapview(filter(nav.paths, cruise == cruise.name),color = "gray20") + 
  mapview::mapview(tdr.paths.sf, color = "green")
