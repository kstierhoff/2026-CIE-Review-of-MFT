...
tdr.fishing.sub <- filter(tdr.fishing, cruise == "202407")

ggplot(filter(tdr.fishing.sub, haul == 133)) +
...
ggplot(filter(tdr.fishing, time.cum < 45)) +
  geom_path(aes(time.cum, Kite, group = haul), colour = "red") +
  geom_path(aes(time.cum, Footrope, group = haul), colour = "blue") +
  facet_wrap(~cruise, nrow = 2)

ggsave(filename = here("Figs/fig_tdr_depths_all_facet.png"),
       width = 10, height = 10)
...
ggsave(filename = here("Figs/fig_tdr_heights_all_facet.png"),
       width = 10, height = 10)
