...
sl.spp <-  c("Engraulis mordax","Sardinops sagax","Etrumeus acuminatus")

dat <- lengths %>% 
  left_join(select(haul, cruise, ship, haul, collection, time = equilibriumTime)) %>% 
...
ggplot(filter(dat, scientificName %in% sl.spp), 
       aes(standardLength_mm, group = season, fill = season, position = "identity")) + 
  geom_histogram() +
